...
import 'captcha_challenge_page.dart' show CaptchaWebConfig;

class MobileCaptchaChallengePage extends StatefulWidget {
  final CaptchaWebConfig config;
  final String baseUrl;

  const MobileCaptchaChallengePage({
    super.key,
  ...
  @override
  State<MobileCaptchaChallengePage> createState() =>
      _MobileCaptchaChallengePageState();
}

class _MobileCaptchaChallengePageState
    extends State<MobileCaptchaChallengePage> {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onWebResourceError: (error) {
            // WebView 会把 Turnstile/Google/Cap 的 iframe、脚本、favicon 等
            // 子资源加载失败也回调到这里，例如 -1: net::ERR_FAILED。
            // 这些并不一定代表验证码主页面失败，直接显示会导致用户在登录页
            // 看到误报的红色错误条。
            //
            // 只有主框架加载失败才作为真正错误展示。
  ...
  Future<void> _loadCaptcha() async {
    ...
    await _controller.loadHtmlString(
      html,
      // 使用站点根地址作为页面来源，避免验证码组件把 /api/v4 当作页面路径。
      // Cloudflare Turnstile 的域名校验看的是 hostname。
      baseUrl: '${Uri.parse(widget.baseUrl).origin}/',
    );
  }
