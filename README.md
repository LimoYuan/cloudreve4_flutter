...
| 文件列表   | ✅ | 列表/网格双视图                            |
| 刷新列表   | ✅ | 原则上增量更新, 但似乎还是重绘了整个UI?              |
| 全屏手势   | ✅ | 右侧左滑返回上级目录, 根目录提示退出                 |
| 文件下载   | ✅ | 原生/浏览器双实现原因、进度监听、断点续传、后台下载          |
...
| 设置页面   | ✅ | 新增缓存管理器用于预览可能会产生的临时文件存放, 增加一些实用的设置项 |
...
>   设置页面
>
>   >   Cloudreve 的控制面板实在是太复杂了, 本来打算实现一些user资料等相关的设置, 后来思虑再三, 发现其实也没这么强需求, 索性就啥都不加了
...
- [ ] ListView 似乎还是完整重绘, 上传插值似乎也还是在完整重绘
